...
int jpeg_decode(const uint8_t* data, int len,
    ...
    JDec* j = (JDec*)kmalloc(sizeof(JDec));
    if (!j) return 0;
...
store:
    /* Assemble final RGB24 image */
    {
        ...
        uint8_t* rgb = (uint8_t*)kmalloc(W * H * 3);
        if (!rgb) goto done;
    ...
    for (int ci = 0; ci < j->ncomp; ci++) {
        if (j->comp[ci].plane) kfree(j->comp[ci].plane);
    }
    kfree(j);
    return ok;
}
