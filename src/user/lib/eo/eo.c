...
efl_class_new(const Efl_Class_Description *desc, const Efl_Class *parent_id, ...)
{
   ...
   /* Build class extensions list */
     {
        ...
        extn_sz = sizeof(_Efl_Class *) * (eina_list_count(extn_list) + 1);        extn_sz = sizeof(_Efl_Class *) * (eina_list_count(extn_list) + 1);
   ...
   /* Prepare mro list */
     {
        ...
        mro_sz = sizeof(_Efl_Class *) * (eina_list_count(mro) + 1);        mro_sz = sizeof(_Efl_Class *) * (eina_list_count(mro) + 1);
   ...
   klass->obj_size = extn_data_off;

     {
        ...
        klass->header.id = new_id;        klass->header.id = new_id;
     }

   if (!_eo_class_initializer(klass))   if (!_eo_class_initializer(klass))
     {
        return NULL;
     }
